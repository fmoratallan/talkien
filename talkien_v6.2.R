...
samp <- read.delim("input_data/clxEntrez.txt", header = T, stringsAsFactors = F)
                                                             ...
                                                             dropdownButton(
                                                               ...
                                                               radioButtons("showCat", "Number of showed pathways:",
                                                                            choices = c("10" = 10, 
                                                                                        ...
                                                                                        "ALL" = "all"),
                                                                            selected = "15"),
                                                               h5(strong("component-wise enrichent:")),
                                                               switchInput("comp",
                                                                           value = F,
                                                                           size = "mini"),
                                                               uiOutput("enrComp"),
...
server <- function(input, output) { 
  ...
  netIni <- reactive({
    ...
    if (input$clx == F) {   
      ...
      data <- samp
      gene_annot <- "entrez.id"
    }
    
    if (NCOL(data)<2) {
      
      output$warning1 <- renderUI({ HTML(paste('<br/>', '<br/>', "Check your input data, lacks list specification column")) })
      output$warning2 <- renderUI({ HTML(paste('<br/>', '<br/>', "Check your input data, lacks list specification column")) })
      output$warning3 <- renderUI({ HTML(paste('<br/>', '<br/>', "Check your input data, lacks list specification column")) })
      output$warning4 <- renderUI({ HTML(paste('<br/>', '<br/>', "Check your input data, lacks list specification column")) })
      output$warning5 <- renderUI({ HTML(paste('<br/>', '<br/>', "Check your input data, lacks list specification column")) })
      netIni <- list(data)
      ...
      if (nrow(data)==0 | nrow(data[!is.na(data[,1]), ])==0) {
        ...
        output$warning5 <- renderUI({ HTML(paste('<br/>', '<br/>', "Check your input data, No header in lists or maybe no entries in lists")) })
        netIni <- list(data)
        ...
        if (length(lists)>8) {
          ...
          output$warning1 <- renderUI({ HTML(paste('<br/>', '<br/>', "Check your input data, No more than 8 different lists supported!")) })
          output$warning2 <- renderUI({ HTML(paste('<br/>', '<br/>', "Check your input data, No more than 8 different lists supported!")) })
          output$warning3 <- renderUI({ HTML(paste('<br/>', '<br/>', "Check your input data, No more than 8 different lists supported!")) })
          output$warning4 <- renderUI({ HTML(paste('<br/>', '<br/>', "Check your input data, No more than 8 different lists supported!")) })
          output$warning5 <- renderUI({ HTML(paste('<br/>', '<br/>', "Check your input data, No more than 8 different lists supported!")) })
          netIni <- list(data)
  ...
  output$arraySel <- renderUI({
    ...
    req(length(netIni)==3)
      ...
      pickerInput(inputId = "list", 
                  ...
                  choices = pairedLists, 
                  multiple = T, 
                  selected = pairedLists, options = list(`none-selected-text` = "default: all pairwise lists selected"))
    #}
  })
  ...
  net <- reactive({
    ...
    req(length(netIni)==3, input$list)
      ...
      if (length(grep("quenya", data$entries))>0) {
        ...
        output$warning5 <- renderUI({ HTML(paste('<br/>', '<br/>', "Check your input data, Quenya annotation not supported!")) })
        net <- list(data)
        ...
        if (length(out_genes$uncomGenes)==0) {
          ...
          output$warning5 <- renderUI({ HTML(paste('<br/>', '<br/>', "Check your input data, all genes common between lists!")) })
          ...
          if(any(unlist(lapply(entryList, length))==0)) {
            ...
            output$warning5 <- renderUI({ HTML(paste('<br/>', '<br/>', "Check your input data, format not allowed")) })
            net <- list(data, out_genes)
            ...
            if(length(anot$annot_genes)==0) {
              ...
              output$warning5 <- renderUI({ HTML(paste('<br/>', '<br/>', "Check your input data, neither receptor nor secreted proteins found!")) })
  ...
  net_clean <- reactive({
    ...
    req(length(net)==10)
      ...
      if(length(E(ppi_subgraph))==0) {
        ...
        output$warning5 <- renderUI({ HTML(paste('<br/>', '<br/>', "Check your input data, no interactions found!")) })
        ...
        if (input$graylinks == T && input$curation == "HighScore") pruned_network <- rbind.data.frame(net$edges[is.na(net$edges$weight), ], net$edges[net$edges$weight>=input$score_sliderHi & !is.na(net$edges$weight), ])
        if (input$graylinks == T && input$curation == "Experimental") pruned_network <- net$edges
        ...
        if(nrow(pruned_network)==0) {
          ...
          output$warning5 <- renderUI({ HTML(paste('<br/>', '<br/>', "No interactions found! - score filter too high -")) })
          ...
          if (input$nettype == "crt") {
            ...
            if(nrow(downcross_network[downcross_network$desc=="main", ])==0) {
              ...
              output$warning5 <- renderUI({ HTML(paste('<br/>', '<br/>', "No interactions found!")) })
              net_clean <- list(ppi_subgraph, pruned_network)
          ...
          if(nrow(downcross_network)==0) {
            ...
            net_clean <- list(pruned_network, nodes, downcross_network, graph_stat, graph_stat2, lists, enr, netstat, edges, lnodes)
  ...
  output$enrComp <- renderUI({
    net_clean <- net_clean()
    req(length(net_clean)==10, req(input$comp))
      netstat <- net_clean[[8]]
  ...
  output$enrTissue <- renderUI({
    net_clean <- net_clean()
    req(length(net_clean)==10, input$tissue)
      netstat <- net_clean[[8]]
  ...
  output$drugs <- renderDataTable({
    ...
    req(length(net_clean)==10)
      ...
      if (input$tabs == '5') {
        ...
        if(nrow(recs)==0) { 
          output$warning5 <- renderUI({ HTML(paste('<br/>', '<br/>', "No Drug interactions found with Receptors provided")) })
        } else {
          datatable(recs)
        }
      }
  ...
  output$oraplot <- renderPlot({
    ...
    req(!is.null(ora))
      ...
      if(is.null(enr)) { 
        output$warning4 <- renderUI({ HTML(paste('<br/>', '<br/>', "No terms selected for enrichment!")) })
      } else {
  ...
  output$downloadDrugTable <- downloadHandler(
    filename = function() {'drugRec_ints.txt'}, content = function(file) {
      recs <- drint$recs
  ...
  output$downloadNetGML <- downloadHandler(
    filename = function() {
      paste('network-', Sys.Date(), '.GraphML', sep = "")
    }, 
    content = function(file) {
      net_clean <- net_clean()
